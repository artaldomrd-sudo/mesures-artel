...
- **Galandaje suelto: reforma 2026-09-07 (pedido del usuario, varios puntos a la vez):**
  ...
  - **Tubos** (`gal_tubo`: `''|'sup'|'lat'|'lat_sup45'`, select "Tubos:" en el menú del galandaje): tubo
    ...
    **El tubo superior horizontal mide SIEMPRE lo mismo que el riel total (ancho de marco)** — regla del
    ...
    resumen dice "largo tubo superior: N mm (= ancho de marco)". **Los tubos van POR FUERA de todo** (regla del usuario,
    lado del tubo lateral confirmado): ni el hueco ni el ancho de marco incluyen el tubo lateral — se dibuja
...
- **Paño Fijo con divisiones dentro de la fachada** (2026-09-17, usuario: "3 paños de 2000 deberían
  ...
  mismos campos que `fachada_din`. `gridCell` dibuja la unión "T" como un marco por paño (cada paño es
  su propio `extrudedPanel`, doble trazo igual al perímetro — pedido del usuario, la línea sola parecía
  vidrio a tope) y la unión a tope como línea fina, y
...
- **Fachada de ESQUINA (en "L")** — pedido del usuario (2026-09-05) a partir de un plano de
  ...
  **UI simplificada (mismo día, "giro 1, giro 2… no se entiende nada de esa frase")**: select
  ...
  caja envolvente del ala (`wMinX/wMaxX/wMinY/wMaxY`) y no de "wingW hacia arriba". Si el ángulo no es
  90° se rotula junto a la esquina; el resumen dice "gira hacia AFUERA a 135°". La elevación desplegada
  no cambia. `verify.mjs`: 8 combinaciones.
